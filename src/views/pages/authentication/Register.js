...
const Register = () => {
    ...
    const ToastContent = ({ name, role }) => (
      <Fragment>
        ...
        <div className='toastify-body'>
          <span>You have successfully Registered in as an {role} user to Vuexy. Now you can start to explore. Enjoy!</span>
        </div>
      </Fragment>
    )
    ...
    return (
     ...
     const PostRegisterData = (e) => {
      if (username === '') {
        ...
        toast('Email must have @ and .')
      } else if (password === '') {
        toast('Password must be more than 4 Digit')
      ...
      fetch(`${baseURL}/register`, {
        ...
        if (data.result === "User Created successfully") {
          ...
          toast('User with the given email address exits, Please Login!')
         }
      }).catch(err => {
          console.log('ERROR ::::', err)
          toast('Make sure you have filled all fields correctly!')
          // toast('Email must have @ and .')
          // toast('Password must be more than 4 digit!')
          // history.push('/)login') 
      })
      ...
      toast('Fill out the form correctly! Password not be less than 5 digit.')
    }

  }

  const CallPostRegisterData = () => {
    PostRegisterData()
  }
  // useEffect(() => {
  ...
  //     useJwt
  //       .register({ usernamee:username, emaill:email, passwordd:password })
  //       .then(res => {
  //         if (res.data.error) {
  //           const arr = {}
        ...
        <Col className='d-flex align-items-center auth-bg px-2 p-lg-5' lg='4' sm='12'>
          <Col className='px-xl-2 mx-auto' sm='8' md='6' lg='12'>
            ...
            <Form className='auth-register-form mt-2' >
              ...
              <Button.Ripple block color='primary' onClick={CallPostRegisterData}>
                Sign up
              </Button.Ripple>
            </Form>
