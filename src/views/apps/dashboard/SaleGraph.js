...
const SaleGraph = () => {
    ...
    datasets: [
      {
        ...
        fill: true,
        lineTension: 0.3,
        backgroundColor: "rgba(300, 204,100, .3)",
        borderColor: "rgb(300, 130, 2)",
        borderCapStyle: "butt",
        borderDash: [],
  ...
  useEffect(() => {
    axios.get(`${baseURL}/stocks/getSaleReport?user_id=${Cookies.get("id")}`)
      .then((records) => {
        const totalArray = []
        const dateArray = []
  ...
  return isLoading ? (
    ...
    <MDBContainer>
      <Row  >
        <Col xs={6} md={6}> <h3 className="mt-5">Sales</h3> </Col>
      </Row>
