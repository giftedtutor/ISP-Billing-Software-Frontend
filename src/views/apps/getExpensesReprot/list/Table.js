...
const UsersList = () => {
  ...
  useEffect(() => {
    setLoading(true)
    axios.get(`${baseURL}/expenses/getExpensesReport?user_id=${Cookies.get("id")}&&pageNo=${pageNo}&&records=${record}&&fromDate=${fromDate && moment(fromDate).format('DD/MM/YYYY')}&&toDate=${toDate && moment(toDate).format('DD/MM/YYYY')}`)
      .then(response => {
